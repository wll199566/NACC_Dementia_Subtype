...
def parse_cli(params):
    ...
    # model
    parser.add_argument('--dropout_rate', type=float, default=params['dropout_rate'], metavar="DP",
                        help="the probability of dropout (default: " + str(params["dropout_rate"]) + ")" )

    parser.add_argument('--leaky_slop', type=float, default=params['leaky_slop'], metavar="LS",
                        help="the leaky slop for the leaky ReLU used in the model (default: " + str(params["leaky_slop"]) + ")" )
    ...
    parser.add_argument('--hidden_dim', type=int, default=params['hidden_dim'], metavar="DFH",
                        help="the dimension of the hidden layer (default: " + str(params["hidden_dim"]) + ")" )

    parser.add_argument("--output_dim", type=int, default=params["output_dim"], metavar="OD",
...
model = FullyConnectedNet(feature_dim=args.feature_dim, hidden_dim=args.hidden_dim, output_dim=args.output_dim, leaky_slop=args.leaky_slop, dropout_rate=args.dropout_rate)
...
################################## Evaluation models #############################################3
# initialize a model
eval_model_macro = FullyConnectedNet(feature_dim=args.feature_dim, hidden_dim=args.hidden_dim, output_dim=args.output_dim, leaky_slop=args.leaky_slop, dropout_rate=args.dropout_rate)
eval_model_micro = FullyConnectedNet(feature_dim=args.feature_dim, hidden_dim=args.hidden_dim, output_dim=args.output_dim, leaky_slop=args.leaky_slop, dropout_rate=args.dropout_rate)
